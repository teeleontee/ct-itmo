...
impl Cell {
    ...
    pub fn direction_to_checked(self, other: Cell, bad_direction: Direction) -> Direction {
        let (dx, dy) = (other.0 - self.0, other.1 - self.1);
        let mut dir = Direction::Right;
        if dx.abs() > dy.abs() {
            if dx > 0 {
                if bad_direction != Right && self.adjacent_unchecked(Right).in_bounds() {
                    dir = Direction::Right
                }
            } else if bad_direction != Direction::Left && self.adjacent_unchecked(Left).in_bounds()
            {
                dir = Direction::Left
            }
        } else if dy > 0 {
            if bad_direction != Direction::Up && self.adjacent_unchecked(Direction::Up).in_bounds()
            {
                dir = Direction::Up
            }
        } else if bad_direction != Direction::Down
            && self.adjacent_unchecked(Direction::Down).in_bounds()
        {
            dir = Direction::Down
        }
        dir
