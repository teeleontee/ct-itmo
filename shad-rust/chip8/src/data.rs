...
impl OpCode {
    ...
    pub fn extract_word(self, index: usize) -> Word {
        ((self.0 >> (index * 8)) & 0xFF) as Word
    }

    pub fn extract_nibble(self, index: usize) -> Nibble {
        Nibble::try_from(((self.0 >> (index * 4)) & 0xF) as u8).unwrap()
    }

    pub fn as_u8s(self) -> [u8; 4] {
        [
            self.extract_nibble(3).as_u8(),
            self.extract_nibble(2).as_u8(),
            self.extract_nibble(1).as_u8(),
            self.extract_nibble(0).as_u8(),
        ]
    }
