...
private val minCut = ArrayList<Int>()
...
private fun reachable(u: Int) {
    if (used[u]) {
        return
    }
    used[u] = true
    for (index in g[u]) {
        if (edges[index].flow - edges[index].capacity < 0) {
            reachable(edges[index].to)
        }
    }
}
...
fun main() {
    ...
    used.fill(false)
    reachable(0)
    (0 until m * 2 step 2).forEach {
        val u = used[edges[it].from]
        val v = used[edges[it].to]
        if (u xor v) {
            minCut.add(it / 2)
        }
    }
    println("${minCut.size} $flow")
    minCut.forEach { print("${it + 1} ") }
}
